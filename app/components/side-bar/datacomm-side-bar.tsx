...
export const DatacommSideBar: React.FC<DatacommSidebarArgs> = ({
 ...
 return (
  <SidebarProvider>
   <Sidebar collapsible="icon" variant="sidebar" className="w-[280px] h-full">
    <SidebarHeader
         ...
         <SidebarMenuItem key={item.title}>
          <SidebarMenuButton
           className={`items-center flex h-10 px-5 py-0 rounded-[10px] hover:bg-white ${
            activeItem === item.title ? 'bg-white border border-[#A5A5AB]' : ''
           }`}
           asChild
           ...
           <Link href={item.url || '#'} className="flex items-center w-full">
            <Image
             ...
             height={16}
            />
            {!collapsed && <span data-testid="menu-title">{item.title}</span>}
